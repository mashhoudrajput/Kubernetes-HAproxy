...
check_ssh() {
    ...
    ssh -o BatchMode=yes -o ConnectTimeout=5 $SSH_USER@$IP "echo 2>&1" > /dev/null
    if [ $? -eq 0 ]; then
...
install_kubectl() {
    ...
    if command -v kubectl &> /dev/null; then
        ...
        sudo apt-get install -y apt-transport-https ca-certificates curl telnet net-tools jq -qq
...
configure_kubectl() {
    ...
    ssh $SSH_USER@$LOAD_BALANCER_IP "sudo mkdir -p /etc/kubernetes"
    scp /tmp/admin.conf $SSH_USER@$LOAD_BALANCER_IP:/etc/kubernetes/admin.conf
...
scp $SSH_USER@$MASTER_NODE_IP:/etc/kubernetes/admin.conf /tmp/admin.conf
