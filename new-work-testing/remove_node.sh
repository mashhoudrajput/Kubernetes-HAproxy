    ...
    if [ "$SERVER_IP" == "$NODE_IP" ]; then
        ...
        if [ "$action" == "1" ]; then
            ...
            if ssh -o BatchMode=yes -o ConnectTimeout=5 $SSH_USER@$SERVER_IP "exit" >/dev/null; then
                ssh $SSH_USER@$SERVER_IP "sudo kubeadm reset -f && sudo systemctl stop kubelet && sudo systemctl stop docker && sudo rm -rf /var/lib/cni/ /var/lib/kubelet/* /var/lib/etcd/* /etc/cni/ /etc/kubernetes/* /var/run/kubernetes/*" >/dev/null
            ...
            members=$(ssh $SSH_USER@$EXISTING_MASTER_IP "etcdctl --endpoints=https://$EXISTING_MASTER_IP:2379 --cacert=$ETCD_CA_CERT --cert=$ETCD_CERT --key=$ETCD_KEY member list" 2>/dev/null)
            member_id=$(echo "$members" | grep "$SERVER_IP" | awk '{print $1}' | tr -d ',')

            if [ -n "$member_id" ]; then
                echo "Removing etcd member with ID: $member_id"
                ssh $SSH_USER@$EXISTING_MASTER_IP "etcdctl --endpoints=https://$EXISTING_MASTER_IP:2379 --cacert=$ETCD_CA_CERT --cert=$ETCD_CERT --key=$ETCD_KEY member remove $member_id" >/dev/null
            ...
            # Drain node from Kubernetes cluster
            if ! ssh $SSH_USER@$SERVER_IP "kubectl drain $NODE_NAME --delete-emptydir-data --force --ignore-daemonsets --timeout=60s" >/dev/null; then
