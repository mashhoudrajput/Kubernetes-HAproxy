...
if ! ssh -o BatchMode=yes -o ConnectTimeout=5 $SSH_USER@$LB_IP 'exit' >/dev/null; then
...
scp /tmp/install_haproxy.sh $SSH_USER@$LB_IP:/tmp/install_haproxy.sh >/dev/null
scp hosts $SSH_USER@$LB_IP:/tmp/hosts >/dev/null
ssh $SSH_USER@$LB_IP 'bash /tmp/install_haproxy.sh' >/dev/null
