...
if ssh $SSH_USER@$EXISTING_MASTER_NODE_IP "kubectl get nodes -o wide | grep -w $SERVER_IP"; then
    ...
    ssh $SSH_USER@$SERVER_IP "mkdir -p \$HOME/.kube && sudo rm -rf \$HOME/.kube/* && sudo cp -i /etc/kubernetes/admin.conf \$HOME/.kube/config && sudo chown \$(id -u):\$(id -g) \$HOME/.kube/config"
fi
